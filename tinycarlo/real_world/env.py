...
class TinyCarloRealWorldEnv(TinyCarloEnv):
    ...
    def __get_env_modules(self, env_name: str):
        modules = [cls for cname, cls in inspect.getmembers(importlib.import_module(f"tinycarlo.real_world.environments.env_{env_name.lower()}"))]
    def __get_camera_dyn(self, modules):
        return [cam for cam in modules if issubclass(cam, Camera)][0]
    def __get_car_dyn(self, modules):
        return [car for car in modules if issubclass(car, Car)][0]
    ...
    def __init__(self, render_mode: Optional[str] = None, config: Optional[Union[str, Dict[str, Any]]] = None):
        ...
        self.real_world_env_name = config["sim"].get("real_world_env", None)
        if self.real_world_env_name is not None:
        ...
        self.car = self.__get_car_dyn(modules)(self.T, self.map, config["car"])
        self.renderer = Renderer(self.map, self.car, self.overview_pixel_per_meter)
        self.camera = self.__get_camera_dyn(modules)(self.map, self.car, self.renderer, config["camera"])
        self.reset()
