...
map_config = {
    ...
    "pixel_per_meter": int(sys.argv[2]) if len(sys.argv) > 2 else 250
}
map = Map(map_config)
bg_color = (255,255,255) if getenv("WHITE") else None
renderer = Renderer(map, overview_pixel_per_meter=map_config["pixel_per_meter"], background_color=bg_color, line_thickness=2, node_names=True)
