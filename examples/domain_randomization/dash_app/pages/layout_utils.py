...
def create_layout(title, outer_file, dashed_file, solid_file, actor_weight_file, critic1_weight_file, critic2_weight_file, critic_loss_file, actor_loss_file, ep_rew_file, cte_file, latent_space):
    ...
    # Create the plots
    outer_fig = px.line(outer_df, x='Distance', y='Step', title="Outer Distance", template="custom_template")
    dashed_fig = px.line(dashed_df, x='Distance', y='Step', title="Dashed Distance", template="custom_template")
    solid_fig = px.line(solid_df, x='Distance', y='Step', title="Solid Distance", template="custom_template")
    ...
    actor_weight_fig = px.line(actor_weight_changes, x='Step', y='Absolute Weight Change', color='Layer', title="Actor Weight Changes", template="custom_template", color_discrete_map=color_discrete_map)
    critic1_weight_fig = px.line(critic1_weight_changes, x='Step', y='Absolute Weight Change', color='Layer', title="Critic1 Weight Changes", template="custom_template", color_discrete_map=color_discrete_map)
    critic2_weight_fig = px.line(critic2_weight_changes, x='Step', y='Absolute Weight Change', color='Layer', title="Critic2 Weight Changes", template="custom_template", color_discrete_map=color_discrete_map)
    ...
    reward_ls_fig = px.scatter(
        ...
        title="Latent Space (Reward)",
        template="custom_template",
        color_continuous_scale="viridis", hover_data=["Reward", "CTE", "Maneuver", "Heading Error"]
    )

    # Plot 2: CTE-Visualisierung
    cte_ls_fig = px.scatter(
        latent_df, x="Dimension1", y="Dimension2", color="CTE",
        title="Latent Space (CTE)",
        template="custom_template",
        color_continuous_scale="rdbu", hover_data=["Reward", "CTE", "Maneuver", "Heading Error"]
    )

    # Plot 3: Maneuver-Visualisierung
    maneuver_ls_fig = px.scatter(
        latent_df, x="Dimension1", y="Dimension2", color="Maneuver",
        title="Latent Space (Maneuver)",
        template="custom_template",
        color_discrete_map={0: "red", 1: "green", 2: "blue"}, hover_data=["Reward", "CTE", "Maneuver", "Heading Error"]
    )

    # Plot 2: CTE-Visualisierung
    heading_error_ls_fig = px.scatter(
        latent_df, x="Dimension1", y="Dimension2", color="Heading Error",
        title="Latent Space (Heading Error)",
        template="custom_template",
        color_continuous_scale="rdbu", hover_data=["Reward", "CTE", "Maneuver", "Heading Error"]
    )

    # Adjust layout for the plots
    for fig in [outer_fig, dashed_fig, solid_fig, critic_loss_fig, actor_loss_fig, ep_rew_fig, cte_fig,
                actor_weight_fig, critic1_weight_fig, critic2_weight_fig, reward_ls_fig, cte_ls_fig, maneuver_ls_fig, heading_error_ls_fig]:
        fig.update_layout(
            autosize=False,
            width=500,
            height=500,
    ...
    return html.Div([
        ...
        html.Div([
            ...
            dcc.Graph(figure=heading_error_ls_fig, style={"width": "500px", "height": "500px"}),
